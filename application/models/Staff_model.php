...
class Staff_model extends CI_Model {
    ...
    public function checkLogin($data) {
        $record = $this->getByEmail($data['email'],md5($data['password']));
        if ($record) {
            
            $roles = $this->getStaffRoles($record->role_id);

            $record->roles = array($roles[0]->name => $roles[0]->id);
    ...
    public function update_role($role_data) {
     ...
     function getOtherOPDStaffbyrole($id) {
        if ($this->session->has_userdata('hospitaladmin')) {
            $doctor_restriction = $this->session->userdata['hospitaladmin']['doctor_restriction'];
            $userdata = $this->customlib->getUserData();
            $role_id = $userdata['role_id'];
